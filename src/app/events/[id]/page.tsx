...
import { Event, Review, Rating, PaginatedResponse } from '@/types';
...
export default function EventDetailPage() {
  ...
  const loadReviews = async () => {
    try {
      const data = await api.getReviews('events', id) as PaginatedResponse<Review>;
  ...
  const loadRating = async () => {
    try {
      const data = await api.getRating('events', id) as Rating;
      setRating(data);
    } catch (err) {
  ...
  const handleSubmitReview = async (data: { rating: number; comment: string }) => {
    ...
    if (editingReview) {
      await api.updateReview(token, 'events', id, editingReview.id, data);
      setEditingReview(null);
    } else {
      await api.createReview(token, 'events', id, data);
    }
  ...
  const handleDeleteReview = async (reviewId: string) => {
    ...
    await api.deleteReview(token, 'events', id, reviewId);
    await loadReviews();
    await loadRating();
  };
